...
jobs:
  qsr-gate:
    ...
    steps:
      ...
      - id: gate
        uses: Sahil170595/quantfit/.github/actions/quantfit-gate@v0.12.15
        with:
          baseline: Qwen/Qwen2.5-1.5B-Instruct
          quant: ./out/qwen2.5-1.5b-awq
          tier: smoke                    # 30pp — gates >=30pp only, and says so
          quantfit-version: "==0.12.15"    # the instrument version is part of the measurement
...
Two independent pins, and you want both: `@v0.12.15` pins the *action*, `quantfit-version`
