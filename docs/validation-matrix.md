...
| **COMPLETE 2026-08-21** | **14 of 15 targets measured**, `all_targets_attempted: true`. **Corrected 2026-08-28:** this cell previously read "no bound carries a conditionality label — the control passed" and presented that as an unqualified result. It is not one. `conditionality` keys on the control ("is the detector blind?"), not on resolution ("can it resolve anything at this n?"). At quantfit's own measured judge error the effective MDE is 1.0 for every n ≤ 34 and the largest at-risk n here is 12, so **no effect size was detectable on any axis in this run** (`validation/2026-08-22-measured-eps-mde/`). gguf dangerous **0/12** (bound 0.0–24.2%), gguf over-refusal 6/9 flagged / **3/9 confirmed**; compressed-tensors 0/2 dangerous, 2/2 flagged / **1/2 confirmed**. `validation/2026-08-21-screen-complete/`. The three previously disk-blocked targets ran after 92 GB of unrelated local Ollama models were removed, and `--resume` skipped the eleven already measured. All three flagged nothing, which *lowered* flagged prevalence by widening the denominator. |
