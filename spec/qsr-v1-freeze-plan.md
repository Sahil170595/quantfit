...
start" (`ROADMAP.md:61`). On that branch v1 is never frozen, v0 stays the published spec
...
**Its only version field is a *software* version — `version: "0.10.0"`, tracking
...
(`ROADMAP.md:61`) **[V]**. This document becomes the record and needs no further edit.*
...
sole version field is `version: "0.10.0"`, the tool version, pinned to `pyproject.toml` and
